...
train_target_csv_file = f'./hw2_data/digits/{target}/train.csv'
...
train_dataset = CustomDataset(target_folder, train_target_csv_file, transform=train_transform, is_train=True)
valid_dataset = CustomDataset(target_folder, valid_target_csv_file, transform=valid_transform, is_train=True)
...
while epoch < epochs:
    ...
    for images, labels in progress_bar:
        ...
        class_output = model(input_data=images)
        loss = label_loss(class_output, labels)
        
        _, class_predicted = class_output.max(1)
        source_label_total += labels.size(0)
        source_label_correct += class_predicted.eq(labels).sum().item()
    ...
    if val_accuracy > best_val_accuracy:
        ...
        checkpoint_path = os.path.join('./model_checkpoint', f'best_upperbound_{target}_{epoch}.pth')
